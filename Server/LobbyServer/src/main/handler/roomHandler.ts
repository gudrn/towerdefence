...
export async function createRoomHandler(buffer: Buffer, session: LobbySession): Promise<void> {
  try {
    ...
    if (!roomId) {
      handleError(
        session,
        new CustomError(ErrorCodes.ROOM_LIMIT_REACHED, '사용 가능한 방 ID가 없습니다.'),
      );
      return;
    }
...
export async function getRoomsHandler(buffer: Buffer, session: LobbySession): Promise<void> {
  ...
  for (const key of keys) {
    ...
    if (serializedRoomData) {
      ...
      if (roomData.state == RoomStateType.PREPARE) roomsData.push(roomData);
    }
  }
...
export async function enterRoomHandler(buffer: Buffer, session: LobbySession): Promise<void> {
  ...
  // 5. 새 유저 입장 정보를 다른 유저들에게 알리기
  {
    ...
    const sendBuffer = PacketUtils.SerializePacket(
      notificationPacket,
      L2G_JoinRoomNotificationSchema,
      ePacketId.L2G_JoinRoomNotification,
      0,
    );
    session.send(sendBuffer);
  }
...
export async function leaveRoomHandler(buffer: Buffer, session: LobbySession): Promise<void> {
  ...
  if (userIndex === -1) {
    handleError(
      session,
      new CustomError(ErrorCodes.LEAVE_ROOM_FAILED, '유저가 방에 존재하지 않습니다.'),
    );
    return;
  }
...
export async function deleteGameRoomHandler(buffer: Buffer, session: LobbySession): Promise<void> {
  try {
    ...
    if (!serializedRoomData) {
      handleError(
        session,
        new CustomError(ErrorCodes.INVALID_ROOM_ID, '유효하지 않은 방 ID입니다.'),
      );
      return;
    }
