...
exports.getDB = exports.db = exports.createDBConnection = exports.app = void 0;
const user_db_1 = require("./../db/user/user.db");
const express_1 = __importDefault(require("express"));
const bcrypt_1 = __importDefault(require("bcrypt"));
const promise_1 = __importDefault(require("mysql2/promise")); // RowDataPacket 추가
const config_1 = require("../config/config");
const redis_init_1 = require("../redis/redis.init");
...
app.post('/login', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // 이메일로 사용자 검색
        const user = yield (0, user_db_1.findUserByEmail)(email);
        if (!user) {
        ...
        // Redis 캐시 확인
        try {
            const doubleLogin = yield redis_init_1.redisManager.getCache(user.nickname);
            if (doubleLogin) {
                res.status(409).json({ message: '현재 접속 중입니다.' });
                return;
            }
            // Redis 캐시에 사용자 로그인 정보 설정
            yield redis_init_1.redisManager.setCache(user.nickname, email); // Redis 캐시 설정
        }
        catch (redisError) {
...
exports.default = app;
// 비밀번호 찾기 API
app.post('/find/password', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // 닉네임 확인이 필요한 경우
        if (nickname) {
            const userNick = yield (0, user_db_1.findUserByNickname)(nickname);
            if (!userNick) {
...
if (!db) {
    exports.db = db = promise_1.default.createPool({
        host: config_1.config.db.host || 'localhost', // config.db.host를 기본값으로 설정
        user: config_1.config.db.user || 'root', // 기본값 추가
        password: config_1.config.db.password || 'password', // 기본값 추가
        database: config_1.config.db.database || 'your_database', // 기본값 추가
...
exports.getDB = getDB;
app.get('/friendsList/email/:email', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [userRows] = yield database.query('SELECT user_id FROM usertable WHERE email = ?', [email]);
        ...
        const [friendsRows] = yield database.query(`SELECT f.friend_id, u.nickname
       FROM friendlist f
...
app.post('/find/email', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // 1. 닉네임으로 사용자 조회
        const user = yield (0, user_db_1.findUserByNickname)(nickname);
        if (!user) {
            res.status(404).json({ message: '회원 닉네임이 존재하지 않습니다.' });
            return;
        }
...
app.post('/find/nickname', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userPass = yield (0, user_db_1.findUserByPassword)(password);
        if (!userPass) {
...
app.get('/userinfo', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
app.patch('/userinfos', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (password) {
            const hashedPassword = yield bcrypt_1.default.hash(password, 10); // 비밀번호 해시
        ...
        yield db.query(`UPDATE usertable SET nickname = ?, password = ? WHERE user_id = ?`, [user.nickname, user.password, user.user_id]);
        // 성공 응답
        res.status(200).json({
