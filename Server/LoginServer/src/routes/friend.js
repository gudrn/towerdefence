...
router.post('/addToFriendList', async (req, res) => {
  ...
  try {
    ...
    const [userCheck] = await connection.query(
      //connenction query=데이터베이스 연결객체의 메서드
    ...
    const [result] = await connection.query(
      //=>insertinfo-friendlist 테이블에 새로운 데이터를 추가/value(?,?)-[user_id, friend_id] 배열이 이 플레이스홀더에 바인딩
    ...
    res.status(201).json({
      //성공응답        //res.status-요청의 성공여부를 클라이언트에 알림
    ...
    //해당 데이터베이스 작업이 독립적으로 처리되고 있기 때문, 긴 코드의 전체 흐름에서,
