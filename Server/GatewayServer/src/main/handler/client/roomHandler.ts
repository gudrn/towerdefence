...
import {
  C2G_ChatMessageRequest,
  C2G_ChatMessageRequestSchema,
  C2G_CreateRoomRequest,
  C2G_CreateRoomRequestSchema,
  C2G_GetRoomListRequestSchema,
  C2G_JoinGameRoomRequestSchema,
  C2G_JoinRoomRequest,
  C2G_JoinRoomRequestSchema,
  G2B_CreateGameRoomRequestSchema,
  G2B_JoinGameRoomRequestSchema,
  G2C_ChatMessageNotificationSchema,
  G2L_CreateRoomRequestSchema,
  G2L_GameStartRequestSchema,
  G2L_GetRoomListRequestSchema,
  G2L_JoinRoomRequestSchema,
...
export function handleC2G_CreateRoomRequest(buffer: Buffer, session: GatewaySession) {
  ...
  const sendBuffer = PacketUtils.SerializePacket(
    requestPacket,
    G2L_CreateRoomRequestSchema,
    ePacketId.G2L_CreateRoomRequest,
    0,
  );
...
export function handleC2G_GameStartRequest(buffer: Buffer, session: GatewaySession) {
  ...
  if (room == undefined) {
    throw new CustomError(
      ...
      `[handleC2G_GameStartRequest] 방을 찾지 못했습니다. ${packet.roomId}`,
    );
  }
  ...
  //1. 로비 서버에게 방 상태 변경 요청
  {
    ...
    const sendBuffer = PacketUtils.SerializePacket(
      requestPacket,
      G2L_GameStartRequestSchema,
      ePacketId.G2L_GameStartRequest,
      0,
    );
  ...
  // 2. 배틀 서버에게 방 생성 요청
  {
    ...
    const sendBuffer = PacketUtils.SerializePacket(
      requestPacket,
      G2B_CreateGameRoomRequestSchema,
      ePacketId.G2B_CreateGameRoomRequest,
      0,
    );
...
export function handleC2G_JoinGameRoomRequest(buffer: Buffer, session: GatewaySession) {
  ...
  if (battleSession == null) {
    throw new CustomError(
      ErrorCodes.SERSSION_NOT_FOUND,
      `[handleC2G_JoinGameRoomRequest] 배틀 세션을 찾지 못했습니다. ${packet.serverId}`,
    );
  }
  ...
  const sendBuffer = PacketUtils.SerializePacket(
    requestPacket,
    G2B_JoinGameRoomRequestSchema,
    ePacketId.G2B_JoinGameRoomRequest,
    0,
  );
...
export function handleChatMessage(buffer: Buffer, session: GatewaySession) {
  ...
  const room = roomManager.getRoom(packet.roomId) as Room;
  // if(room == undefined) {
  ...
  const sendBuffer = PacketUtils.SerializePacket(
    responePacket,
    G2C_ChatMessageNotificationSchema,
    ePacketId.G2C_ChatMessageNotification,
    0,
  );
