...
import {
  C2G_PlayerPositionUpdateRequestSchema,
  C2G_PlayerUseAbilityRequestSchema,
...
export function handleC2G_PlayerPositionUpdateRequest(buffer: Buffer, session: GatewaySession) {
  ...
  const sendBuffer = PacketUtils.SerializePacket(
    requestPacket,
    G2B_PlayerPositionUpdateRequestSchema,
    ePacketId.G2B_PlayerPositionUpdateRequest,
    0,
  );
...
export function handleC2G_TowerBuildRequest(buffer: Buffer, session: GatewaySession) {
  ...
  const sendBuffer = PacketUtils.SerializePacket(
    requestPacket,
    G2B_TowerBuildRequestSchema,
    ePacketId.G2B_TowerBuildRequest,
    0,
  );
  battleSession.send(sendBuffer);
}
...
export function handleC2G_UseSkillRequest(buffer: Buffer, session: GatewaySession) {
  ...
  const sendBuffer = PacketUtils.SerializePacket(
    requestPacket,
    G2B_UseSkillRequestSchema,
    ePacketId.G2B_UseSkillRequest,
    0,
  );
  battleSession.send(sendBuffer);
}
...
export function handleC2G_PlayerUseAbilityRequest(buffer: Buffer, session: GatewaySession) {
  ...
  const sendBuffer = PacketUtils.SerializePacket(
    requestPacket,
    G2B_PlayerUseAbilityRequestSchema,
    ePacketId.G2B_PlayerUseAbilityRequest,
    0,
  );
  battleSession.send(sendBuffer);
}
