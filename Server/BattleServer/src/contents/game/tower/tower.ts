...
---------------------------------------------*/
   ...
   */
  protected attackTarget(targetData: { monster: SkillUseMonster; distance: number }) {
   ...
   * @param target 타겟 몬스터
   */
  protected abstract processAttack(target: SkillUseMonster): void;
  ...
  * @returns {boolean} 
  ...
  applyAttackBuff() {
    if (!this.isBuffed) {
      ...
      console.log(
        `[버프 적용] ${this.getPrefabId()}: ${this.originalAttackDamage} -> ${this.attackDamage}`,
      );
  ...
  removeAttackBuff() {
    ...
    console.log(
      `[버프 해제] ${this.getPrefabId()}: ${this.attackDamage} -> ${this.originalAttackDamage}`,
    );
