...
// 타워 생성 알림
export function createTowerBuildNotificationPacket(
  tower: { prefabId: string; towerPos: PosInfo },
  ownerId: string,
  sequence: number,
) {
...
// 타워 공격 모션
export function createTowerAttackMotionPacket(
  towerId: string,
  monsterPos: PosInfo,
  travelTime: number,
) {
...
// 타워 버프 적용 알림
export function createTowerBuffNotificationPacket(
  towers: { getId: () => string }[],
  buffType: string,
  isBuffed: boolean,
): Buffer {
